...
하루치 판매만 담은 **간소 CSV**(날씨 컬럼 없음)에, 그날 공통 메타(행사/신메뉴/시나리오)를 **body 필드**로 함께 전달한다. body 메타는 해당 날짜의 모든 행에 적용된다.

**Request** — `multipart/form-data`
| 파트 | 타입 | 필수 | 설명 |
|---|---|---|---|
| `file` | file(CSV) | ✅ | 헤더: `날짜,요일,품목,구분,판매수량` |
| `storeId` | int | ✅ | 매장 ID |
| `eventFlag` | boolean | ✅ | 행사 여부(그날 전체 적용) |
| `newMenuFlag` | boolean | ✅ | 신메뉴 여부(그날 전체 적용) |
| `scenarioNote` | string | ❌ | 비고/시나리오 (예: `주말+맑음->수요 높음`) |
...
```json
{ "success": true,
  "data": { "appliedDate": "2026-06-28", "accepted": 2, "rejected": 0,
    "eventFlag": false, "newMenuFlag": true, "errors": [] } }
```
...
## 부록 — 변경 이력
- 신규 `POST /ingest/sales/daily` (하루치 CSV + body 메타: eventFlag/newMenuFlag/scenarioNote).
