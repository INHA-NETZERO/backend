...
| 항목               | 값                                                            |
| ------------------ | ------------------------------------------------------------- |
| 권장 스택          | Python 3.10+, FastAPI, AWS Bedrock 이용, LightGBM             |
...
POST /v1/order-recommendation
{
  ...
  "salesHistory": {                            // 판매내역 CSV (S3 presigned GET URL)
    "presignedUrls": [
      "https://zerowave.s3.ap-northeast-2.amazonaws.com/sales/store1/sales-2026-05.csv?X-Amz-Algorithm=...&X-Amz-Expires=600&X-Amz-Signature=...",
      "https://zerowave.s3.ap-northeast-2.amazonaws.com/sales/store1/sales-2026-04.csv?X-Amz-..."
    ],
    "format": "sales_csv_v1"                   // 컬럼 규약(§3.5)
  },
...
| salesHistory.presignedUrls[] | string[] | ✅   | 판매내역 CSV의 **S3 presigned GET URL**. AI가 다운로드해 추론모델 입력으로 사용(§3.5) |
| salesHistory.format          | string   | ✅   | CSV 컬럼 규약 식별(`sales_csv_v1`)                                                    |
...
### 3.5 판매내역 CSV (presigned URL) 처리

백엔드는 정형 피처(`rows[].features`)에 더해, **장기 판매 이력 CSV의 S3 presigned GET URL**을 함께 전달한다. AI는 이 URL로 CSV를 직접 내려받아 추론모델 입력(과거 시계열·계절성 등)으로 사용한다.

- **다운로드:** `presignedUrls[]`의 각 URL을 HTTP GET. 인증정보가 URL에 포함되어 별도 키 불필요. **유효시간 제한(예: 10분)** 내 사용.
- **CSV 컬럼 규약(`sales_csv_v1`):** `날짜, 요일, 날씨, 기온, 강수mm, 행사, 신메뉴, 품목, 구분, 판매수량, 비고_시나리오` (UTF-8, BOM 가능). 백엔드 `GET /export/sales.csv`(backend_spec §5.8)와 동일.
- **다건:** 여러 달 파일이 올 수 있음(예: 최근 N개월). AI가 병합·정렬해 사용.
- **실패 처리:** URL 만료/다운로드 실패 시 해당 이력 없이 정형 피처만으로 추론하거나, 불가하면 `baseline_v1`로 폴백.
- **보안:** presigned URL은 **읽기 전용·시간제한**. AI는 저장하지 않고 추론 후 폐기(stateless).
...
POST /v1/forecast
{
  ...
  "salesHistory": {                            // 판매내역 CSV presigned URL (§3.5와 동일)
    "presignedUrls": [ "https://zerowave.s3...sales-2026-05.csv?X-Amz-..." ],
    "format": "sales_csv_v1"
  },
...
> §3과 차이: `coverage` 없음, `weather`는 **배열이 아닌 단일 객체**, `rows[]`에 발주정책(orderCycleDays/leadTimeDays) 불필요. `salesHistory`(판매내역 CSV)는 §3.5 규약과 동일하게 전달.

| 경로            | 타입   | 필수 | 설명                                        |
| --------------- | ------ | ---- | ------------------------------------------- |
| storeId         | int    | ✅   | 매장 ID                                     |
| targetDate      | date   | ✅   | 예측 대상일(다음날)                         |
| salesHistory    | object | ✅   | 판매내역 CSV presigned URL(§3.5)            |
