...
| Method | Path | 용도 |
|---|---|---|
| POST | `/v1/forecast` | 커버기간 품목별 분위 수요예측 |
...
## 3. `POST /v1/forecast` — 수요예측

발주 도래 품목들에 대해, **커버기간(리드타임+발주주기) 일별 분위예측**을 반환한다.
백엔드가 일별 예측을 합산(Σ)해 발주량을 산출하므로, AI는 **일자별 p10/p50/p90**를 주는 것이 기본이다.
...
## 4. `POST /v1/generate` — 자연어 설명 (sLLM)
...
## 9. 참고 예시 (전체 흐름)
우유 품목, 발주주기 7일·리드타임 1일 → 커버기간 8일 시나리오의 요청/응답 전 과정은
